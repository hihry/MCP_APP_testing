...
import { JSONPLACEHOLDER_SPEC, DRIFTED_USER_RESPONSE } from "./constants.js";
import {
  createSession,
...
Call after the user has confirmed their selection in the test-review MCP App.`,
    inputSchema: {
      ...
      simulate_drift: z.boolean()
        .optional()
        .default(false)
        .describe("Set true to simulate schema drift on GET /users/{id} — demonstrates SelfHealingEngine."),
    },
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
  ...
  async ({ approved_test_ids, simulate_drift }) => {
    ...
    // Find GET /users/{id} test for drift simulation
    const driftTargetTest = testsToRun.find(
      (t) => t.method === "GET" && t.path === "/users/{id}"
    );
    ...
    const driftNote = simulate_drift && failed > 0
      ? `\n⚠️  Schema drift detected on ${driftTargetTest?.endpoint ?? "GET /users/{id}"}!\n   Run 'detect_and_heal' to analyse and patch the failing assertion.`
      : "";
